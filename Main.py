...
def process():
    ...
    ebflag=False
    mflag=False
    def is_speaking(prev_img, curr_img, debug=False, threshold=700, width=400, height=400):
        """
        Args:
            prev_img:
            curr_img:
    ...
    while True:
        ...
        for rect in rects:
            ...
            if is_speaking(prev_mouth_img, mouth_img, threshold=400,
                                    debug=True):
                cv2.putText(frame, "Lip Movement", (50, 150), 1, 7, (255, 0, 0))
                sflag=True
                i += 1
        ...
        for face in faces:
            ...
            if blinking_ratio > 4.7:
                ebflag=True
                cv2.putText(frame, "Eye BLINKING", (50, 150), 1, 7, (255, 0, 0))
    ...
    #    cv2.imshow("delta", frame_delta)
        ...
        if sflag or ebflag or mflag:
            playsound("alarm.wav")
            msg="Your patient got conciousness from coma"
            
            sms.process(msg,"9742516319","8747028227")
            sflag=False
            ebflag=False
            mflag=False
